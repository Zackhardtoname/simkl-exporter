...
def make_csv(data):
	with open('./simklData.csv', mode='w') as movie_file:
		...
		for movie in data['movies']:
			try: year = str(movie['movie']['year'])
			except: year = ""
			try: imdb = str(movie['movie']['ids']['imdb'])
			except: imdb = ""
			try: tmdb = str(movie['movie']['ids']['tmdb'])
			except: tmdb = ""
			try: watched = str(movie['last_watched_at'])[:10]
			except: watched = ""
			try: rating10 = str(movie['user_rating']) if str(movie['user_rating']) != "None" else ""
			except: rating10 = ""
			try: movietitle = str(movie['movie']['title'])
			except: movietitle = ""
			writer.writerow({
				'Year':year,
				'imdbID':imdb,
			...
			print("Imported "+movietitle+" ("+year+"), watched on "+watched+(", rated "+rating10 if len(rating10) > 0 else ""))
